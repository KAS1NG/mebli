  ...
  return (
    ...
    >
      <article className={styles.card} role="article">
        {/* Знижка / бейдж */}
        {hasDiscount && (
          <span className={`${styles.label} ${styles.sale}`} aria-label="Знижка">
            <span>Знижка</span>
        ...
        <div className={styles.info}>
          ...
          {hasDiscount ? (
            <div
              ...
              title={`Економія ${discountValue?.toLocaleString('uk-UA')} грн`}
            >
              <span className={styles.oldPrice}>
                {product.price.toLocaleString('uk-UA')} грн
              </span>
              <span className={styles.newPrice}>
                {newPrice?.toLocaleString('uk-UA')} грн
              </span>
              <span className={styles.discount}>
                -{Math.round(brand * 100)}%
              </span>
            </div>
          ) : (
